...
InfoBar::InfoBar(MessageProcessor& processor) : mp(processor) {
    connector = std::make_unique<OSCConnect>(processor, ip_host.getIPAddress(), ip_console.getIPAddress(), ip_tmix.getIPAddress());
...
InfoBar::~InfoBar() { connector->close(-1); }

void InfoBar::paint(juce::Graphics& g) {
...
void InfoBar::startButtonClicked() {
